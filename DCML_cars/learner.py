...
class metric_learner(object):
    def __init__(self, conf, inference=False):
        ...
        if not inference:
            ...
            if conf.use_dataset == 'Cars':
                train_path = conf.datapth+'cars_train'+str(conf.cross_valid)+'.txt'
  
                self.dataset = MSBaseDataSet(conf, train_path, transform=conf.transform_dict['rand-crop'], mode='RGB')
                
                valid_path = conf.datapth+'cars_valid'+str(conf.cross_valid)+'.txt'
    ...
    def train(self, conf):
        ...
        while self.epoch <= conf.epochs:
            ...
            if self.epoch % self.evaluate_every ==0 and self.epoch != 0:
                ...
                if recall_ks_val[0] > bestval_testrecall:
                    
                    bestval_testrecall = recall_ks_val[0]
    ...
    def save_model(self, conf):
        ...
        save_file = os.path.join(conf.save_folder, 'valid_ckpt_crossid{cross_id}_seed{seed}_regcof{regcof}_atcof{atcof}_wcof{w_cof}.pth'.format(
                cross_id=conf.cross_valid, seed=conf.seed, regcof=conf.reg_cof, atcof=conf.at_cof, w_cof=conf.w_cof))
    ...
    def test(self, conf):
        ...
        if conf.use_dataset=='Cars':
            ...
            if conf.use_dataset == 'Cars':
                dataset = MSBaseDataSet(conf, conf.datapth+'cars_test.txt', transform=conf.transform_dict['center-crop'], mode='RGB')
