    ...
    def _load_data(self):
        with open(self.img_source, 'r') as f:
            for line in f:
                ...
                self.label_list_temp.append(_label) #label list 0, 1, 0, 2, ...

    def _relabel(self):
        label_set = set(self.label_list_temp)
        label_set = list(label_set)
        label_id = dict()
        for j, label_ in enumerate(label_set):
            label_id[label_] = j
        for label in self.label_list_temp:
            self.label_list.append(label_id[label])

    def _build_label_index_dict(self):
        index_dict = defaultdict(list)
        for i, label in enumerate(self.label_list):
            index_dict[label].append(i)#index_dict label and index, e.g., label0: index
        return index_dict
