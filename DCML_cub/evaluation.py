...
def Recall_at_ks(sim_mat, data_name=None, query_ids=None, gallery_ids=None):
    ...
    neg_nums = np.zeros(m)
    for i in range(m):
        x = sim_mat[i]#[s11, s12, .., s1n]

        pos_max = np.max(x[gallery_ids == query_ids[i]])#ben lei li zuida de 

        neg_num = np.sum(x > pos_max)#biede lei bi pos_max da de
        neg_nums[i] = neg_num

    for i, k in enumerate(k_s):
        if i == 0:
            temp = np.sum(neg_nums < k)#k=1
            num_valid[i] = temp
        else:
...
def MAP_R(sim_mat, gallery_ids):

    """
    :param sim_mat:
    : r is the number of samples per class - 1
    :param gallery_ids

    Compute AMP@R
    """
    
    R_dict = collections.Counter(gallery_ids)#samples per class
    max_r = np.max( np.array(list( R_dict.values() ), dtype=int) ) - 1 # minus-1 for query
    ...
    sim_mat += np.diag(np.ones(num) * minval)

    indices = np.argsort(-sim_mat, axis=1)[:, : max_r] #num*r
    
    labels = gallery_ids[indices] #num*r
    
    Rs = np.array( list( map(R_dict.get, gallery_ids) ), dtype = int ) - 1
    maps = np.zeros((num, max_r), dtype=float)
    
    for i in range(1, max_r+1): #precision@k, 1, 2, 3, ..., max_r
        
        k = i - 1 #qian idx lie
        
        mask_r = 1.0*(Rs >= i) #valid samples mask
        
        mask = 1.0*(gallery_ids==labels[:,k]) #if the kth correct
        
        cur_label = labels[:, :k+1] #num*k
        targets = gallery_ids.reshape(-1, 1)        
        corrects = 1.0*(cur_label==targets)
        cor_num = np.sum(corrects, axis =1)
        precision = cor_num/i
        precision = precision*mask
        precision = precision*mask_r
        maps[:,k] = precision
        
    maprs = np.sum(maps, axis =1)/Rs
    mapr = np.mean(maprs)
    return mapr


def R_Precision(sim_mat, gallery_ids):
    ...
    for i in range(1, max_r+1): #precision@k, 1, 2, 3, ..., max_r
        ...
        precision = precision*mask_r
        sums += np.sum(precision)
        total_samples += np.sum(mask_r)
        
    assert len(gallery_ids)==total_samples    
    rp = sums/len(gallery_ids)
