...
def MAP_R(sim_mat, gallery_ids, labs):
    # start_time = time.time()
    # print(start_time)
    """
    :param sim_mat:
    ...
    Rs = np.array( list( map(R_dict.get, labs) ), dtype = int ) - 1
    ...
    for i in range(1, max_r+1): #precision@k, 1, 2, 3, ..., max_r
        ...
        mask = 1.0*(labs==labels[:,k]) #if the kth correct
        
        cur_label = labels[:, :k+1] #num*k
        targets = labs.reshape(-1, 1)  #num*k      
...
def R_Precision(sim_mat, gallery_ids, labs):
    # start_time = time.time()
    # print(start_time)
    """
    :param sim_mat:
